...
These optional values can be passed as keys of the ``config`` dictionary in the ``GitLabProviderFactory`` constructor.
...
* ``allowed_project_visibilities``: Only projects with one of these configured visibilities will be included when syncing a user's project list. Other projects will be ignored. By default, all visibilities are enabled. To set a value, use a list of strings. For example: ``["private", "internal"]``.
