...
if (sync_button) {
  sync_button.addEventListener("click", function () {
    ...
    async function fetchWithTimeout(url, options, timeout = 100000) {
...
if (repositories) {
  for (const repo of repositories) {
    const toggleSwitch = repo.getElementsByClassName("repo-switch").item(0);
...
// ON OFF toggle a11y
const $onOffToggle = $(".toggle.on-off.repo-switch");
