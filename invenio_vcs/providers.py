...
from invenio_vcs.errors import MultipleWebhooksError, RemoteAccountDataNotSet
...
class RepositoryServiceProviderFactory(ABC):
    ...
    @property
    @abstractmethod
    def oauth_remote_config(self) -> dict[str, Any]:
        """
        Returns a dictionary as the config of the OAuth remote app for this provider.

        The config of the app is usually based on the config variables provided in the
        constructor. The format is based on the specification from invenio-oauthclient,
        see the example: https://github.com/inveniosoftware/invenio-oauthclient/blob/master/invenio_oauthclient/config.py
        """
    ...
    @cached_property
    def oauth_remote(self):
        """The corresponding remote OAuth client app."""
        return LocalProxy(lambda: current_oauthclient.oauth.remote_apps[self.id])

    @property
    @abstractmethod
    def provider_specific_config(self) -> dict:
        """Returns a configuration dictionary with special options that are specific to a given provider.

        The value could, for example, be passed through the constructor. Provider classes can use this value
        internally. The structure is heavily provider-dependent so it should not be used externally except
    ...
    def webhook_event_to_generic_release(
        ...
        """Returns the data of the release as extracted from the raw JSON payload of a webhook event, in generic form.

        The repository is not returned since VCS providers might not include this in the payload, and some only include
        partial fields. To get the full repository, it needs to be queried separately.
        """
        raise NotImplementedError
...
class RepositoryServiceProvider(ABC):
    ...
    @cached_property
    def oauth_remote_account(self):
        """Returns the OAuth Remote Account corresponding to the user's authentication with the provider."""
        return RemoteAccount.get(self.user_id, self.factory.oauth_remote.consumer_key)
    ...
    def oauth_remote_token(self):
        ...
        token = RemoteToken.get(self.user_id, self.factory.oauth_remote.consumer_key)
    ...
    def webhook_url(self):
        ...
        webhook_token = ProviderToken.query.filter_by(
            id=self.oauth_remote_account.extra_data["tokens"]["webhook"]
        ).first()
        if webhook_token:
    ...
    def is_configured_webhook(self, url: str | None):
    ...
    def get_configured_webhook(self, repository_id: str) -> GenericWebhook | None:
        """Get the webhook for which `is_configured_webhook` is true.

        Repositories should only have one valid configured webhook for this Invenio instance.
        This method will check all registered webhooks for the `repository_id`. If there are
        multiple, a `MultipleWebhooksError` will be raised.
        """
        ...
        for hook in webhooks:
            if self.is_configured_webhook(hook.url):
                if found_hook is not None:
                    # Raise an error if this is not the first matching webhook.
                    raise MultipleWebhooksError(repository_id)
    ...
    def list_repository_contributors(
        ...
        from a remote source). The list may include non-human contributors
        (e.g. automated tools or organisations).

        The list must be returned in order of descending number of commits. If
        this is not possible (e.g. due to a limitation of the VCS API), it may
    ...
    def update_webhook(self, repository_id: str, hook_id: str) -> bool:
        ...
        Returns True if the webhook is successfully updated or no updated is needed.
        """
        raise NotImplementedError
