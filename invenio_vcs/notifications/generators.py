...
class RepositoryUsersRecipient(RecipientGenerator):
    ...
    This `RecipientGenerator` is provided by Invenio VCS for use by higher-level implementors.
    For example, it can be used during release processing/publishing to notify members of the
    repository of the release's status.

    This generator does _not_ include all users on the VCS who have access to the repository;
    it does not have access to this information. Instead, it only includes users who were reported
    as having access during the last sync _and_ have connected their VCS account to the Invenio
    instance.
    """
    ...
    def __call__(self, notification, recipients: dict):
        ...
        user_repo_associations = repository.list_users()

        user_ids: set[str] = set()
        for association in user_repo_associations.mappings():
