# SPDX-FileCopyrightText: 2014-2025 CERN.
...
def register_ui_routes(blueprint):
    ...
    @blueprint.route("/repository/<path:vcs_repo_id>")
    @login_required
    @require_vcs_connected()
    @vcs_error_handler()
    def get_repository(provider, vcs_repo_id):
        ...
        db_repo = svc.get_repository(vcs_repo_id)
        latest_release = svc.get_repo_latest_release(db_repo.id)
        default_branch = svc.get_repo_default_branch(vcs_repo_id)
        releases = svc.list_repo_releases(db_repo)
        release_url = svc.provider.factory.url_for_release
        new_release_url = svc.provider.factory.url_for_new_release(db_repo.full_name)
        new_citation_file_url = svc.provider.factory.url_for_new_file(
            db_repo.full_name, default_branch or "main", "CITATION.cff"
        )
...
def register_api_routes(blueprint):
    ...
    def enable_repository(provider, repository_id):
        ...
        svc = VCSService.for_provider_and_user(provider, current_user.id)
        create_success = svc.enable_repository(repository_id, community_id=community_id)
