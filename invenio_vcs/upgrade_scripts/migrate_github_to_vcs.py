...
"""Migration script for the old GitHub-only integration to the new generic VCS integration."""
...
remote_account_table = sa.table(
    ...
    # Changed for invenio-oauthclient v6: https://github.com/inveniosoftware/invenio-oauthclient/pull/360
    sa.Column(
        "extra_data",
                ...
                if matching_db_repo_id is None:
                    ...
                    db.session.execute(
                        vcs_repositories_table.insert().values(
                            ...
                            # We also didn't store this before
                            record_community_id=None,
                            created=datetime.now(tz=timezone.utc),
                            updated=datetime.now(tz=timezone.utc),
                        )
                    )
...
def verify_alembic_version(expected_revision: str):
    ...
    with db.engine.connect() as connection:
        ...
        if expected_revision not in current_revs:
            secho(
                "The invenio_vcs Alembic branch is not at the latest revision. Please upgrade it before continuing.",
                fg="red",
            )
            sys.exit(1)
...
def execute_upgrade():
    """Execute all of the steps for the upgrade to InvenioVCS."""
    secho("Starting Invenio-GitHub -> Invenio-VCS data migration...", fg="green")
